          ...
          <view class="w-100 d-flex flex-column">
            ...
            <view class="d-flex justify-content-between align-items-center" style="margin-bottom: 30rpx;">
              <view class="font-size-sm text-color-assist">
                {{ item.productioned_time }}
              </view>
              <view class="d-flex font-size-sm text-color-base align-items-center">
                <view style="margin-right: 10rpx;">共{{ item.goods.length }}件商品，实付</view>
            ...
            <view class="d-flex align-items-center justify-content-end">
              <view>
                ...
                <button type="primary" v-if="item.status ==15" plain size="mini" @tap.stop="deleteOrder(item)">
                  删除订单
                </button>
                <button type="primary" v-if="item.status ==12" plain size="mini" @tap.stop="cancelOrder(item)">
                  取消订单
                </button>
              </view>
            </view>
